...
public class DianpaoZhuangDeterminer implements ZhuangDeterminer {
	...
	public void determineZhuang(Ju ju) throws Exception {
		...
		DianpaoMajiangPanResult lastPanResult = (DianpaoMajiangPanResult) ju.findLatestFinishedPanResult();
		...
		if (huPlayerIds.size() > 1) {
			...
			// 最后一个摸牌的人做庄家
			DianpaoMajiangLastMoActionPlayerRecorder lastMoActionPlayerRecorder = ju
					.getActionStatisticsListenerManager().findListener(DianpaoMajiangLastMoActionPlayerRecorder.class);
